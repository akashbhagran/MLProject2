...
# Configure Celery to use your chosen broker, e.g., Redis
celery_app = Celery('db_tasks', broker='redis://redis:6379/0')

def init_db():
    with sql.connect('fruits') as conn:
        cursor = conn.cursor()
...
def insert_fruit(mass, width, height, colour, fruit, label):
    with sql.connect('fruits.db') as conn:
        cursor = conn.cursor()
...
app = Celery('db_tasks', broker = 'redis://redis:6379/0')
...
def read_data():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM FRUITS ORDER BY ROWID DESC LIMIT 10")  # Example read
